"""環氧樹脂工作站 V6.4.23 — 1K/2K配方設計·ML黏度預測·物料管理·多語言"""

# DPI 感知必須在 import tkinter 前宣告，否則高 DPI 螢幕 (Win10 125%/150% 縮放)
# 會啟用 DPI virtualization，造成 winfo_rootx()/rooty() 與滑鼠座標不一致。
import sys as _sys
if _sys.platform == 'win32':
    try:
        import ctypes
        # PerMonitorV2 → SystemAware → 古老 fallback chain
        try:
...
def _detect_font():
    """Select a pan-CJK font that supports Traditional Chinese, Simplified
    Chinese, AND Japanese kana/kanji. Falls back gracefully if a font is missing.

    Why this matters: 'Microsoft JhengHei' (Win) and 'PingFang TC' (macOS) are
    optimized for Chinese only — rendering Japanese kana with these fonts on
    a system that lacks Japanese language pack causes Tk to throw, which can
    silently destroy popups (e.g. dropdowns close immediately on open).
    """
    p = _sys.platform
    # Candidate chain by platform — prefer fonts that natively cover CJK + kana
    if p == 'darwin':
        candidates = [
            "PingFang SC",        # macOS, full pan-CJK incl. kana
            "Hiragino Sans",      # macOS, JP-first but renders CN ok
            "Hiragino Sans GB",
            "PingFang TC",        # CN-only fallback
            "Helvetica",
        ]
    elif p == 'win32':
        candidates = [
            "Yu Gothic UI",       # Win10/11 default, full JP + CN coverage
            "Meiryo UI",          # Win7+, full JP + CN coverage
            "Microsoft YaHei UI", # CN-first but covers JP kana
            "Microsoft JhengHei", # TC-only (was the buggy default)
            "Segoe UI",
        ]
    else:
        candidates = [
            "Noto Sans CJK JP",   # JP variant covers TC/SC well
            "Noto Sans CJK TC",
            "Noto Sans CJK SC",
            "Source Han Sans",
            "DejaVu Sans",
    ...
    # Probe via tkinter.font.families() if available; otherwise return first
    try:
        import tkinter
        import tkinter.font
        # tkinter.font.families() requires a root; create a transient one
        ...
        if root is None:
            ...
            created = True
        try:
            available = set(tkinter.font.families())
        finally:
            if created:
                try:
                    root.destroy()
        ...
        # If tk isn't ready (e.g. import-time on headless systems), just trust the chain
        pass
...
# 字型涵蓋探測 — 各語言代表性字符
_LANG_PROBE_CHARS = {
    'zh_TW': '繁體中文：龜鬱靈鑫',  # 繁中特有/罕用字
    'zh_CN': '简体中文：龟郁灵鑫',  # 簡中對應字
    'ja':    'ひらがな・カタカナ・漢字',  # 平假名 + 片假名 + 漢字
    'en':    'The quick brown fox',  # ASCII Latin
}

def _probe_font_coverage(lang):
    """測試當前 _FONT_FAMILY 是否能渲染指定語言的字符。

    技巧：用 tkfont.measure() 量測代表字符寬度。若系統字型無法渲染，
    Tk 會回傳 0 寬度或 .notdef glyph 寬度（通常等於 ASCII 'M' 寬度）。
    我們比較目標字符與已知良好字符（'M'）的寬度比例判斷。
    ...
    try:
        ...
        if root is None:
            ...
            created = True
        try:
            f = _tkfont.Font(root=root, family=_FONT_FAMILY, size=12)
            probe = _LANG_PROBE_CHARS.get(lang, '')
            if not probe:
                return True
            # 量測 'M' 作為基準寬度
            m_w = f.measure('M')
            if m_w <= 0:
                return False
            probe_w = f.measure(probe)
            expected_min = m_w * len(probe) * 0.6  # CJK 至少 0.6 倍 ASCII 寬度
            return probe_w >= expected_min
        finally:
            if created:
                try:
                    root.destroy()
        ...
        return True  # 探測失敗時樂觀假設正常，不阻擋使用者
...
# 預設語言為英文 (Latin 字符所有作業系統都能渲染)；切到其他語言時若字型缺字會彈出提醒
_CURRENT_LANG = "en"
...
# 字型缺失警告 (彈窗用目標語言顯示，引導使用者安裝語言包)
"font_warn_title":            {"zh_TW":"字型不足提醒", "zh_CN":"字体不足提醒", "en":"Font Coverage Notice", "ja":"フォント不足のお知らせ"},
"font_warn_zh_TW":            {"zh_TW":"您的系統可能未安裝完整的繁體中文字型，部分字符可能無法正確顯示。\n\n建議安裝：Microsoft JhengHei (繁體中文支援)\nWindows: 設定 → 時間與語言 → 語言 → 新增中文 (台灣)\nmacOS: 系統設定 → 一般 → 語言與地區 → 新增繁體中文\n\n仍要切換語言？", "zh_CN":"您的系統可能未安裝完整的繁體中文字型...", "en":"Your system may not have full Traditional Chinese font coverage; some characters may not display correctly.\n\nRecommended: Install Microsoft JhengHei (Traditional Chinese)\nWindows: Settings → Time & language → Language → Add Chinese (Taiwan)\nmacOS: System Settings → General → Language & Region → Add Traditional Chinese\n\nSwitch language anyway?", "ja":"システムに繁体字中国語フォントが不足している可能性があり、一部の文字が正しく表示されない場合があります。\n\n推奨: Microsoft JhengHei をインストール\nWindows: 設定 → 時刻と言語 → 言語 → 中国語 (台湾) を追加\n\n言語を切り替えますか？"},
"font_warn_zh_CN":            {"zh_TW":"您的系統可能未安裝完整的簡體中文字型，部分字符可能無法正確顯示。\n\n建議安裝：Microsoft YaHei (簡體中文支援)\nWindows: 設定 → 時間與語言 → 語言 → 新增中文 (簡體)\nmacOS: 系統設定 → 一般 → 語言與地區 → 新增簡體中文\n\n仍要切換語言？", "zh_CN":"您的系统可能未安装完整的简体中文字体，部分字符可能无法正确显示。\n\n建议安装：Microsoft YaHei (简体中文支持)\nWindows: 设置 → 时间和语言 → 语言 → 添加中文 (简体)\nmacOS: 系统设置 → 通用 → 语言与地区 → 添加简体中文\n\n仍要切换语言？", "en":"Your system may not have full Simplified Chinese font coverage; some characters may not display correctly.\n\nRecommended: Install Microsoft YaHei (Simplified Chinese)\nWindows: Settings → Time & language → Language → Add Chinese (Simplified)\nmacOS: System Settings → General → Language & Region → Add Simplified Chinese\n\nSwitch language anyway?", "ja":"システムに簡体字中国語フォントが不足している可能性があり、一部の文字が正しく表示されない場合があります。\n\n推奨: Microsoft YaHei をインストール\nWindows: 設定 → 時刻と言語 → 言語 → 中国語 (簡体) を追加\n\n言語を切り替えますか？"},
"font_warn_ja":               {"zh_TW":"您的系統可能未安裝日文字型，假名 (あいうえお) 與日文漢字可能無法正確顯示。\n\n建議安裝：Yu Gothic UI 或 Meiryo UI\nWindows: 設定 → 時間與語言 → 語言 → 新增日文\nmacOS: 系統設定 → 一般 → 語言與地區 → 新增日文\n\n仍要切換語言？", "zh_CN":"您的系统可能未安装日文字体，假名 (あいうえお) 与日文汉字可能无法正确显示。\n\n建议安装：Yu Gothic UI 或 Meiryo UI\nWindows: 设置 → 时间和语言 → 语言 → 添加日文\nmacOS: 系统设置 → 通用 → 语言与地区 → 添加日文\n\n仍要切换语言？", "en":"Your system may not have Japanese fonts installed; kana (あいうえお) and Japanese kanji may not display correctly.\n\nRecommended: Install Yu Gothic UI or Meiryo UI\nWindows: Settings → Time & language → Language → Add Japanese\nmacOS: System Settings → General → Language & Region → Add Japanese\n\nSwitch language anyway?", "ja":"システムに日本語フォントがインストールされていない可能性があり、かな (あいうえお) や日本語漢字が正しく表示されないことがあります。\n\n推奨: Yu Gothic UI または Meiryo UI をインストール\nWindows: 設定 → 時刻と言語 → 言語 → 日本語を追加\nmacOS: システム設定 → 一般 → 言語と地域 → 日本語を追加\n\n言語を切り替えますか？"},
...
class RoundedPopup:
    ...
    def show(self, cb, items, on_select):
        ...
        # 不使用 -transparentcolor (Win10 高 DPI 下會導致整個 popup 變透明而看不見)
...
class AppleDropdown(ctk.CTkButton):
    ...
    def __init__(self, parent, values=None, variable=None, command=None,
        ...
        # ⚠️ DO NOT RENAME `_user_command` ⚠️
        # 此屬性必須叫 _user_command，不能改回 _command。原因：
        # super().__init__(command=self._toggle) 呼叫 CTkButton.__init__，後者內部
        # 會執行 self._command = self._toggle 將父類的 command 覆蓋進去。若這裡
        # 用 self._command 儲存使用者 callback，會被父類同名屬性覆蓋掉，導致
        # _on_pick 呼叫 self._command(value) 實為 self._toggle(value)，因參數
        # 不符拋 TypeError 被吞，使用者 callback 永遠不執行。
        ...
        self._global_bind_id = None
        self._click_token = None
        self._focus_out_bind_id = None
        self._focus_out_root = None
        ...
        # 不要為這個 button 額外加 <Button-1> click fallback。曾有版本加過
        # `self.bind('<Button-1>', self._fallback_check, add='+')` 補強 click，
        # 但延遲檢查 + 計時閾值在較慢系統會誤判 popup 已開啟為「需要再 toggle」
        # 而立即關閉它。CTkButton 的 command 已能正確觸發 _toggle，無需 fallback。
    ...
    def _show_dropdown(self):
        ...
        # 夾住 popup 在螢幕邊界內 — Win10 + DPI awareness 模式下，跨出邊界的
        # overrideredirect Toplevel 會直接不被 DWM 渲染。
        try:
            sw = self.winfo_screenwidth()
            sh = self.winfo_screenheight()
            if x + w > sw - 4:
                x = max(4, sw - w - 4)
            n_rows = min(10, len(self._values))
            popup_h = n_rows * 32 + 12
            if y + popup_h > sh - 4:
                # 空間不夠就改顯示在按鈕上方
                y = max(4, self.winfo_rooty() - popup_h - 2)
        except Exception:
            pass
        ...
        def _handler(event, _token=token):
            if self._click_token is not _token:
                return
            self._on_global_click(event)
        # 延後到下一輪事件迴圈再 bind_all — 避免「觸發 dropdown 的那次 click event」
        # 被自己的 handler 收到。在高 DPI 螢幕上配合 winfo_* 暫時回傳錯誤座標，
        # 會讓 popup 被誤判為點擊在外、立刻關閉 (現象：popup 一閃消失)。
        def _delayed_bind():
            if self._click_token is token and self._popup is not None:
                self._global_bind_id = root.bind_all('<Button-1>', _handler, '+')
        self.after_idle(_delayed_bind)

        # 主視窗失焦時自動關閉 popup (避免 popup 懸浮在其他 app 上)。
        # 用 200ms 延遲二次確認 — 因為點 popup 內 button 時主視窗會短暫失焦，
        # 立即關閉會在 button command 跑完前銷毀 popup → 選擇失效。
        def _focus_out_handler(event, _token=token):
            if self._click_token is not _token:
                return
            try:
                ...
                pass
            def _check_and_close(_token=_token):
                if self._click_token is not _token:
                    return
                try:
                    if root.focus_displayof() is not None:
                        return  # 主視窗仍有 widget 持焦點 = 沒真的離開 app
                except Exception:
                    pass
                self._close_dropdown()
            try:
                root.after(200, _check_and_close)
            except Exception:
                pass
        try:
            self._focus_out_bind_id = root.bind('<FocusOut>', _focus_out_handler, '+')
        except Exception:
            self._focus_out_bind_id = None
        self._focus_out_root = root
    ...
    def _on_global_click(self, event):
        ...
        try:
            ...
            pw = self._popup.win
            # 強制 popup widget 完成 idle 任務，確保 winfo_* 取到正確座標
            # (popup 剛 map 完時這些值可能還是 0)
            try:
                pw.update_idletasks()
            except Exception:
                pass
            pwx, pwy = pw.winfo_rootx(), pw.winfo_rooty()
            pww, pwh = pw.winfo_width(), pw.winfo_height()
            # popup 還沒映射時不誤判點擊在外
            if pww <= 1 or pwh <= 1:
                return
            if (pwx <= cx <= pwx + pww and pwy <= cy <= pwy + pwh):
                return
    ...
    def _unbind_global(self):
        self._click_token = None
        self._global_bind_id = None
        # 同步解除 FocusOut 監聽，避免殘留 binding 干擾下次 popup
        try:
            if getattr(self, '_focus_out_bind_id', None) and getattr(self, '_focus_out_root', None):
                self._focus_out_root.unbind('<FocusOut>', self._focus_out_bind_id)
        except Exception:
            pass
        self._focus_out_bind_id = None
        self._focus_out_root = None
...
class _AppleDropdownPopup:
    ...
    def show(self, x, y, w, values, current, on_pick):
        ...
        self.win.wm_overrideredirect(True)
        # 用 topmost 而不是 transient(top) — Win10 DWM 對 overrideredirect+transient
        # 組合行為不穩定，可能導致 popup 不被渲染。
        self.win.wm_attributes('-topmost', True)
        # 不使用 -transparentcolor — Win10 + DPI awareness 下會讓整個視窗變透明。
        self.win.configure(bg='white')
        ...
        # 防禦性渲染 — 若主要字型缺字導致 button 建立失敗 (例如系統無日文字型
        # 卻要顯示「日本語」)，改用 Tk 預設字型重試。
        for val in values:
            is_sel = (val == current)
            try:
                btn = ctk.CTkButton(
                ...
                try:
                    ...
                    import sys as _s
                    print(f"[AppleDropdownPopup] Failed to render value {val!r}: {e}", file=_s.stderr)
...
class EpoxyApp:
    ...
    def _build_window(self):
        ...
        self.root.title(f"{T('app_title')} V6.4.23")
        ...
        ctk.CTkLabel(self.top_bar, text="v6.4.23", font=self.font_ver, text_color=_C.TEXT_TER).pack(side='left', padx=(6,0), pady=(6,0))
    ...
    def _on_lang_change(self, choice):
        ...
        lang = rev.get(choice, "en")
        # 切換前探測字型涵蓋。若目標語言無法正確渲染就跳警告，警告訊息用目標語言
        # 顯示讓使用者切過去後也看得懂，並串接英文版做雙保險。
        if lang != _CURRENT_LANG and not _probe_font_coverage(lang):
            warn_key = f"font_warn_{lang}"
            saved = _CURRENT_LANG
            try:
                _CURRENT_LANG = lang
                msg_target = T(warn_key)
                _CURRENT_LANG = 'en'
                msg_en = T(warn_key)
            finally:
                _CURRENT_LANG = saved
            full_msg = msg_target + "\n\n— English —\n" + msg_en if msg_target != msg_en else msg_en
            _CURRENT_LANG = lang
            title = T("font_warn_title")
            _CURRENT_LANG = saved
            confirm = messagebox.askyesno(title, full_msg)
            if not confirm:
                self.lang_cb.set(LANG_DISPLAY.get(_CURRENT_LANG, "English"))
                return
        _save_lang(lang)
...
if __name__ == "__main__":
    ...
    root = ctk.CTk()
    # 同步 Tk scaling 與系統 DPI，補強 SetProcessDpiAwareness 後的渲染一致性
    if _sys.platform == 'win32':
        try:
            import ctypes
            # 取得目前主螢幕 DPI
            ...
            scaling = dpi_x / 72.0  # Tk 慣例：72 DPI = scaling 1.0
